...
type GasConsumptionContext int

const (
	GasConsumptionContextExecute GasConsumptionContext = iota
	GasConsumptionContextInstantiate
	GasConsumptionContextMigrate
...
type GasConsumptionHook interface {
	ConsumeGas(ctx sdk.Context, consumptionContext GasConsumptionContext, caller, consumerContract sdk.AccAddress, gasConsumed uint64)
}

func WithGasConsumptionHook(hook GasConsumptionHook) Option {
	return optsFn(func(k *Keeper) {
		k.gasConsumptionHooks = append(k.gasConsumptionHooks, hook)
	})
}