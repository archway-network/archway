#!/bin/bash
# init_two_chainz_relayer creates two archwayd chains and configures the relayer
...
# Ensure archwayd is installed
if ! [ -x "$(which archwayd)" ]; then
  echo "Error: wasmd is not installed. Try running 'make build-wasmd'" >&2
  exit 1
fi
...
# Stop existing archwayd processes
killall archwayd &> /dev/null
...
./one_chain.sh archwayd $chainid0 ./data 26657 26656 6060 9090
./one_chain.sh archwayd $chainid1 ./data 26557 26556 6061 9091
...
rly config init
rly config add-chains configs/archwayd/chains
rly config add-paths configs/archwayd/paths
