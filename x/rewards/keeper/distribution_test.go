...
// TestWithdrawRewards tests the withdraw operation.
func (s KeeperTestSuite) TestWithdrawRewards() {
	keeper := s.chain.GetApp().RewardsKeeper
	accAddr := s.chain.GetAccount(0).Address

	// Withdraw nothing
	s.Run("Withdraw empty rewards", func() {
		rewardsDistributed := keeper.WithdrawRewards(s.chain.GetContext(), accAddr)
		s.Assert().Empty(rewardsDistributed)
	})

	// Withdraw all rewards
	s.Run("Withdraw all rewards", func() {
		rewardsExpected := sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 250))

		// Mint rewards for the later withdrawal
		{
			ctx := s.chain.GetContext()
			s.Require().NoError(s.chain.GetApp().MintKeeper.MintCoins(ctx, rewardsExpected))
			s.Require().NoError(s.chain.GetApp().BankKeeper.SendCoinsFromModuleToModule(ctx, mintTypes.ModuleName, rewardsTypes.ModuleName, rewardsExpected))
		}

		// Create a few records within different blocks
		{
			ctx := s.chain.GetContext()
			keeper.GetState().RewardsRecord(ctx).
				CreateRewardsRecord(
					accAddr,
					sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 50)),
			...
			keeper.GetState().RewardsRecord(ctx).
				CreateRewardsRecord(
					accAddr,
					sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 200)),
					ctx.BlockHeight(), ctx.BlockTime(),
				)
			s.chain.NextBlock(0)
			...
			ctx = s.chain.GetContext()
			s.Require().Len(
				keeper.GetState().RewardsRecord(ctx).GetRewardsRecordByRewardsAddress(accAddr),
				2,
			)
		}
		...
		// Withdraw and check balance
		{
			...
			accBalanceBefore := s.chain.GetBalance(accAddr)
			rewardsReceived := keeper.WithdrawRewards(s.chain.GetContext(), accAddr)

			accBalanceAfter := s.chain.GetBalance(accAddr)
			accBalanceDiff := accBalanceAfter.Sub(accBalanceBefore)

			s.Assert().Equal(rewardsExpected.String(), rewardsReceived.String())
			s.Assert().Equal(rewardsExpected.String(), accBalanceDiff.String())
		}

		// Check records pruning
		{
			s.Require().Empty(keeper.GetState().RewardsRecord(s.chain.GetContext()).GetRewardsRecordByRewardsAddress(accAddr))
		}
	})
}
