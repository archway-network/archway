...
func (suite *HooksTestSuite) SetupEnv() {
	...
	app, ctx, acc := Setup(suite.T())

	// create the echo contract
	contractKeeper := wasmKeeper.NewDefaultPermissionKeeper(&app.Keepers.WASMKeeper)
	contractID, _, err := contractKeeper.Create(ctx, acc.GetAddress(), counterWasm, nil)
	suite.NoError(err)
	counterContractAddr, _, err := contractKeeper.Instantiate(
		ctx,
		contractID,
		acc.GetAddress(),
		nil,
	...
	contractID, _, err = contractKeeper.Create(ctx, acc.GetAddress(), echoWasm, nil)
	suite.NoError(err)
	echoContractAddr, _, err := contractKeeper.Instantiate(
		ctx,
		contractID,
		acc.GetAddress(),
		nil,
...
func (suite *HooksTestSuite) TestOnRecvPacketEcho() {
	...
	err := suite.App.Keepers.BankKeeper.SendCoins(suite.Ctx, suite.TestAddress.GetAddress(), escrowAddress, sdk.NewCoins(testEscrowAmount))
	suite.NoError(err)

	// since ibc-go >= 7.1.0 escrow needs to be explicitly tracked
	if transferKeeper, ok := any(&suite.App.Keepers.TransferKeeper).(TransferKeeperWithTotalEscrowTracking); ok {
	...
	ics4Middleware := ibc_hooks.NewICS4Middleware(
		suite.App.Keepers.IBCKeeper.ChannelKeeper,
		wasmHooks,
	)

	// create the ibc middleware
	transferIBCModule := ibctransfer.NewIBCModule(suite.App.Keepers.TransferKeeper)
...
func (suite *HooksTestSuite) TestOnRecvPacketCounterContract() {
	...
	count, err := suite.App.Keepers.WASMKeeper.QuerySmart(
		suite.Ctx,
